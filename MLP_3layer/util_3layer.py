...
class MLP(nn.Module):
    def __init__(self, input_size, dim_1, dim_2, dropout_rate=0.5):
        super().__init__()
        # Define hidden layers with configurable dimensions
        self.layer_1 = nn.Linear(in_features=input_size, out_features=dim_1)
        self.layer_2 = nn.Linear(in_features=dim_1, out_features=dim_2)
        self.layer_3 = nn.Linear(in_features=dim_2, out_features=1)
    ...
    def forward(self, x):
        ...
        x = self.bn2(x)
        x = self.dropout(x)
        x = self.layer_3(x)  # No activation for the output layer
        return x
...
def run_MLP(data, feat_name, sypt_name, dim_1, dim_2, dropout_rate, lr, decay, verbose=True):
    
    print(f'>>> Using {feat_name} for {sypt_name}')
    print(f'Dim 1: {dim_1}, Dim 2: {dim_2}, Dropout: {dropout_rate}, Initial learning rate: {lr}, L2 penalty: {decay}')
    ...
    # Train the model
    reg_model = MLP(input_size=X_train.shape[1], dim_1=dim_1, dim_2=dim_2, dropout_rate=dropout_rate).to(device)
