...
import pandas as pd
import argparse
from util_3layer import split_data, run_MLP
...
def parse_args():
    ...
    parser.add_argument('--dim_1', type=int, default=64, help='Number of neurons in the first hidden layer')
    parser.add_argument('--dim_2', type=int, default=32, help='Number of neurons in the second hidden layer')
...
if __name__ == "__main__":
    ...
    torch_dict = run_MLP(df, sypt_name=args.item, feat_name=args.feat, 
                          dim_1=args.dim_1, dim_2=args.dim_2, dropout_rate=args.dropout, 
                          lr=args.lr, decay=args.decay, verbose=args.verbose)
    
    if args.save:
        # save the model 
        save_path = (
            f"results/models/model-MLP3_item-{args.item}_feat-{args.feat}_param-"
            f"{{dim1-{args.dim_1}_dim2-{args.dim_2}_dropout-{args.dropout}_lr-{args.lr}_l2-{args.decay}}}.pth"
        )
...
# # run the model 
# df = data_g if 'P' not in 'P1' else data
# torch_dict = run_MLP(df, feat_name='m-HuBERT', sypt_name='P1', 
#                      dim_1=20, dim_2=5, dropout_rate=0.4, lr=0.001, decay=0.001,
#                      verbose=True)


# # save the model 
# save_path = "results/models/MLP3_item-P1_feat-m-HuBERT_param-{dim1-20_dim2-5_dropout-0.4_lr-0.001_l2-0.001}.pth"
