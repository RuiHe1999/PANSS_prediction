...
for f in tqdm(files):
    ...
    for task in tasks:
        if not task.strip():
            continue
        # 1. [count/total], symptom, feature, dim, dropout, lr, l2
        header_match = re.search(
            r"\[(\d+)/(\d+)\] \[RUNNING\] ([^ ]+) ([^ ]+) \| Dim: (\d+), Dropout: ([\deE\.\-]+), LR: ([\deE\.\-]+), L2: ([\deE\.\-]+)",
            task
        )
        if header_match:
            count, total, symptom, feature, dim, dropout, lr, l2 = header_match.groups()
        else:
            continue
        ...
        records.append({
            ...
            "model": "2-layer MLP", 
            "feature": feature,
            "parameter": {"dim": int(dim), "dropout": float(dropout), 
